...
namespace PPM_step_2
{
    class Employee
    {
        ...
        List<Role> employeesRoles = new List<Role>();
        ...
        public Employee()
        {
            try
            {
                ...
                if (flag == false)
                {
                    //Create new class of role  
                    /* Role role = new Role(rid);
                     role.AddRole();
                     employeesRoles.Add(role);            //add the roles into the local list
                     Program.roleList.Add(role); */            // add the role into main rolelist
        ...
        //para contructor
        public Employee(int eid, int rid, string fname, string lname, string email, string phone, string addrss)
        {
            empolyeeId = eid;
            roleId = rid;
            Role role = new Role(rid);
            role.AddRole();
            employeesRoles.Add(role);
        ...
        public void showAllRoles()
        {
            if (roleId != 0)
            {
                ...
                {
                    if (roleId == Employees_roles[i].roleId)
